...
io.on("connection", (socket) => {
 ...
 socket.on("positivePostAdd", async (data) => { // Исправлено название события
  console.log("===--- positivePostAdd ---====", data);
  try {
    // Обновляем только поле positiveLikes, увеличив его на 1
    ...
    // Отправляем обновленный пост через сокет
    socket.emit("postPositiveEdited", {
      message: "Post positive edited successfully!", // Сообщение
      post: POST, // Обновленный пост
    });
  } catch (error) {
    // Обрабатываем ошибку и отправляем сообщение об ошибке через сокет
    socket.emit("error", {
