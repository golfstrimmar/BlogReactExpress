...
io.on("connection", (socket) => {
  ...
  socket.on("updateComment", async (data) => {
    ...
    try {
      ...
      const updateData = {
        text: text || comment.text, // Сохраняем предыдущий текст если не предоставлен новый
        updatedAt: new Date(), // Добавляем метку времени обновления
      };
      // Выполнение обновления
      const updatedComment = await Comment.findByIdAndUpdate(
        commentId,
        { $set: updateData },
        { new: true, runValidators: true } // Возвращаем обновленный документ и проверяем валидацию схемы
      );
      ...
      if (Object.keys(updateQuery).length > 0) {
        updatedComment = await Comment.findByIdAndUpdate(updateQuery, {
          new: true,
        });
      } else {
