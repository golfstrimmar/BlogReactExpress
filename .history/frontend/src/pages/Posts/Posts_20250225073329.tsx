...
const Posts = () => {
  ...
  const selectItems = [
    { name: "Newest First", value: "desc" as "desc" }, // Указание типа явно
    { name: "Oldest First", value: "asc" as "asc" }, // Указание типа явно
  ];
        ...
        <div className="posts-list">
          {currentPosts.length > 0 ? (
            currentPosts.map((post: PostInterface) => (
              <Post key={post._id} post={post} fullText: boolean;/>
            ))
          ) : (
