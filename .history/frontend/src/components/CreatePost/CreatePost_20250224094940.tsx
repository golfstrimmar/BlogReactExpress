...
const CreatePost: React.FC = () => {
  ...
  import { useNavigate } from "react-router-dom";
  import { AppDispatch, RootState } from "../store"; // Путь к вашему store
  import { addPost } from "../store/postsSlice"; // Путь к вашему slice
  ...
  const MyComponent: React.FC = () => {
    ...
    const navigate = useNavigate();
    const socket = useSelector((state: RootState) => state.socket); // Предполагается, что socket находится в Redux
    const [message, setMessage] = useState<string>("");
    const [openModalMessage, setOpenModalMessage] = useState<boolean>(false);
    ...
    useEffect(() => {
      ...
      // Обработчик события "postCreated"
      socket.on("postCreated", (data: SocketData) => {
    ...
    return (
      <div>
        ...
        {openModalMessage && <div>{message}</div>}
      </div>
    );
  };
