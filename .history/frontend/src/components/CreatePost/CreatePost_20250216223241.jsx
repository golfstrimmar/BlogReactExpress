...
const CreatePost = () => {
 ...
 const handleImageUpload = async () => {
   if (image) {
     ...
     try {
       ...
       setImageUrl(imageUrl); // Сохраняем ссылку на изображение в стейте
       console.log("Updated imageUrl state:", imageUrl); // Логируем, что ссылка установлена в стейт
     } catch (error) {
 ...
 const handleSubmit = async (e) => {
   ...
   if (image) {
     await handleImageUpload(); // Ждем, пока загрузка изображения завершится

     // Проверка, что imageUrl успешно получен
     if (imageUrl) {
       console.log("Submitting with imageUrl:", imageUrl);

       // Отправка поста с imageUrl
       socket.emit("createPost", {
         title,
         text,
         tags: selectedTags,
         imageUrl, // Используем загруженную ссылку на изображение
       });
     } else {
