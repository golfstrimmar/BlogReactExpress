  ...
  const handleSubmit = (e) => {
   ...
   socket.emit("createPost", { title, text }, (response) => {
     if (response.message === "Post created successfully!") {
       // Здесь отправляется успешное событие создания поста
   ...
   // Логика обработки события, когда сервер отправляет успешный ответ
   socket.on("postCreated", (data) => {
     console.log(data.message); // Печать сообщения из события
     // Если нужно, можно выполнить дополнительные действия по завершению поста.
   });
