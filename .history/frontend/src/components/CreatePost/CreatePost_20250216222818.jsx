...
const CreatePost = () => {
  ...
  if (image) {
    ...
    try {
      ...
      console.log("Cloudinary response imageUrl:", imageUrl);  // Логируем ссылку на изображение

      setImageUrl(imageUrl);  // Сохраняем ссылку на изображение
      console.log("Updated imageUrl state:", imageUrl);  // Логируем, что мы сохранили URL
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (image) {
      await handleImageUpload(); // Ждем завершения загрузки изображения

      // Если ссылка на изображение существует, отправляем пост
      if (imageUrl) {
      ...
      // Если изображения нет, отправляем только текстовые данные
      console.log("Emitting createPost with:", {
        title,
        text,
        selectedTags,
