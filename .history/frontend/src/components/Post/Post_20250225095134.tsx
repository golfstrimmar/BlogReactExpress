...
const Post: React.FC<PostProps> = ({ post, fullText }) => {
  ...
  useEffect(() => {
    if (socket) {
      socket.on("postDeleted", (data: SocketData) => {
        if (data) {
      ...
      socket.on("commentDeleted", (data: SocketData) => {
        if (data) {
          console.log(
      ...
      socket.on("error", (error: SocketData) => {
  ...
  // ------------------------------------
  const handlerСommentEdit = (el: Comment) => {
    navigate(`/comment/${el._id}/edit`);
  };
  // ------------------------------------
  const handlerСommentDelete = (el: Comment) => {
    if (socket) {
